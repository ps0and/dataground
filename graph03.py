...
st.header("파이 차트")
st.write("파이 차트는 :red[**데이터의 비율**]을 시각적으로 표현한 그래프입니다. 각 항목이 전체에서 차지하는 비율을 쉽게 비교할 수 있어, 비율 기반의 데이터 시각화에 유용합니다.")
st.write("파이 차트를 사용하면 지역별 인구 비율, 영업이익 비율 등 데이터를 비율로 쉽게 시각화할 수 있습니다.")
